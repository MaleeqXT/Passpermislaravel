...
<head>
    ...
    <title>Contrat de Formation PPF</title>
    <style>
        body {
...
<body>
    @php
        function renderCheckbox($label, $value) {
            ...
            return "<span class='checkbox'>{$res}</span> &nbsp;  <span>{$label}</span>";
        }
    @endphp
    <!-- Page 1 -->
    <div style="border: 4px solid #00000088;margin-bottom: 30px">
    ...
    <table width="100%" border="1" cellspacing="0" cellpadding="5">
        <tr>
            <th width="65%">Entre :</th>
            <th width="35%">Et :</th>
        </tr>
        <tr>
            <td>Raison ou dénomination sociale de l'établissement : SAS PASSPERMISFACILE</td>
            <td>Mme : <b>{{ $training['student']['user']['name'] }}</b></td>

        </tr>
        <tr>
            <td>Forme juridique et montant du capital social (pour les sociétés commerciales) : SA au capital de 2000€</td>
            <td>Né(e) le <b>
                {{ Carbon::parse($training['student']['user']['date_naissance'])->format('d/m/Y') }}
            </b></td>
        </tr>
        <tr>
            <td>Le cas échéant, préciser la qualité : N° RCS de COMPIÈGNE sous le numéro B 979 143 294</td>
            <td>Adresse : <b>{{  $training['student']['user']['adresse'] ?? '........' }}</b></td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td>Mail : <b>{{ $training['student']['user']['email'] }}</b></td>
        </tr>
        <tr>
            <td>N° d'identification à la TVA : FR40979143294</td>
            <td>N° tél : <b>{{ $training['student']['user']['phone'] }}</b></td>
        </tr>
        <tr>
        ...
        <tr>
            <td>Ci-après désigné(e) l'école de conduite : <b>PASSPERMISFACILE</b></td>
            <td>Ci-après désigné : <b>l'élève</b></td>
        </tr>
    </table>
    ...
    <p>
        @if(!empty($evaluation['created_at']))
            L'évaluation de l'élève a été réalisée le: <strong>{{ Carbon::parse($evaluation['created_at'])->format('d/m/Y') }}</strong>
        @else
            L'évaluation de l'élève n'a pas encore été renseignée.
        @endif
    </p>
    <p>
        @if(!empty($review_monitor['estimation']))
            À l'issue de cette évaluation, le nombre d'heures prévisionnel de formation pratique est de : <strong>{{ $review_monitor['estimation'] }} heures</strong>.
        @else
            Le nombre d'heures prévisionnel de formation pratique n'est pas encore défini.
        @endif
    </p>
    ...
    <p>Le présent contrat entre en vigueur entre les parties au jour de sa signature pour une durée de 12 mois, soit jusqu'au : <strong>{{ Carbon::parse($created_at)->addMonths(12)->format('d/m/Y') }}</strong></p>
    <p>Les tarifs, les prix détaillés et les termes du contrat ne sont pas révisables pendant toute la durée du contrat sauf modification législative ou réglementaire. Le contrat peut faire l'objet d'une prolongation par voie d'avenant à l'initiative de l'une ou l'autre des parties. Le présent contrat porte sur une durée de un an.</p>
    ...
    <table>
        ...
        <tr>
            <td>{{ $training['offer']['name']}} : {{ $training['offer']['final_price'] }}€</td>
            <td></td>
        </tr>
        {{-- <tr>
            <td><strong>Passpermis à la carte</strong></td>
            <td></td>
        </tr>

        <tr>
            <td>Fabrication Permis : 39,90€</td>
            <td></td>
        </tr>
        <tr>
            <td>RDV Pédagogique (2h) : 99€ (Manuelle ou Automatique)</td>
            <td></td>
        </tr>
        <tr>
            <td>Accompagnement Examen : Boîte Manuelle (forfait 20H = 59 € forfait 10H = 150 €)</td>
            <td></td>
        </tr>
        <tr>
            <td>Boîte Automatique (forfait 20H = 69 € forfait 13H = 150 €)</td>
            <td></td>
        </tr>
        <tr>
            <td>Livre de code : 10 €</td>
            <td></td>
        </tr>
        <tr>
            <td>Conduite Supervisée 2h : 150 € (Manuelle ou Automatique)</td>
            <td></td>
        </tr>
        <tr>
            <td><strong>Passpermis Heure d'évaluation</strong></td>
            <td></td>
        </tr>
        <tr>
            <td>Boîte manuelle 1h à 38€</td>
            <td></td>
        </tr>
        <tr>
            <td>Boîte automatique 1h à 45€</td>
            <td></td>
        </tr>
        <tr>
            <td><strong>Passpermis boîte manuelle</strong></td>
            <td></td>
        </tr>
        <tr>
            <td>forfait 5h : 245€</td>
            <td></td>
        </tr>
        <tr>
            <td>forfait 10h : 470€</td>
            <td></td>
        </tr>
        <tr>
            <td><strong>Passpermis boîte automatique</strong></td>
            <td></td>
        </tr>
        <tr>
            <td>forfait 5h : 275€</td>
            <td></td>
        </tr>
        <tr>
            <td>forfait 13h : 640€ (en 2 fois 690€)</td>
            <td></td>
        </tr>
        <tr>
            <td>forfait 5h : 900€ (en 2 fois 980€ en 3 fois 1050€)</td>
            <td></td>
        </tr>
        <tr>
            <td><strong>Passpermis conduite accompagnée</strong></td>
            <td></td>
        </tr>
        <tr>
            <td>forfait 20h + 4h RDV pédagogique : 1080 €(en 2 fois 1180€ en 3 fois 1250€)</td>
            <td></td>
        </tr>
        <tr>
            <td><strong>Passpermis code de la route</strong></td>
            <td></td>
        </tr>

        <tr>
            <td>code intensif (10h en salle avec un moniteur) : 150€</td>
            <td></td>
        </tr>
        <tr>
            <td><strong>Passpermis découverte</strong></td>
            <td></td>
        </tr>
        <tr>
            <td>inscription, code en ligne, examen code, 1h dévaluation : 89€</td>
            <td></td>
        </tr>
        <tr>
            <td><strong>Passpermis Turbo manuelle</strong></td>
            <td></td>
        </tr>
        <tr>
            <td>forfait 6h + examen en accéléré : 790€</td>
            <td></td>
        </tr>
        <tr>
            <td>forfait 20h + examen en accéléré : 1490€</td>
            <td></td>
        </tr>
        <tr>
            <td><strong>Passpermis Turbo automatique</strong></td>
            <td></td>
        </tr>
        <tr>
            <td>forfait 13h + examen en accéléré : 990€</td>
            <td></td>
        </tr>
        <tr>
            <td>forfait 20h + examen en accéléré : 1690€</td>
            <td></td>
        </tr> --}}
    </table>

    <p style="font-size: 22px;background-color: #f0f0f0;">TOTAL TTC EN EUROS    : <b style="color: crimson">{{ $training['offer']['final_price'] }}€</b></p>

    <h2>6. PROGRAMME ET DÉROULEMENT DE LA FORMATION</h2>
    <p>L'école de conduite s'engage à délivrer à l'élève une formation théorique et pratique conforme aux dispositions législatives et réglementaires en vigueur. Les objectifs de la formation sont précisés dans les quatre compétences de formation du livret d'apprentissage remis à l'élève le jour de la signature du contrat. Ces compétences sont les suivantes : <strong>MAÎTRISER LE MANIEMENT</strong> du véhicule dans un trafic faible ou nul ; <strong>APPRÉHENDER</strong> la route et circuler dans des conditions normales ; <strong>CIRCULER</strong> dans des conditions difficiles et partager la route avec les autres usagers ; <strong>PRATIQUER</strong> une conduite autonome, sûre et économique.</p>
    ...
    <h2>A. 1. Programme de formation en vigueur</h2>
    ...
    <h2>A. 2. Déroulement de la formation</h2>
    ...
    <h2>A. 3. Moyens pédagogiques et techniques</h2>
    ...
    <h2>A. 4. Accompagnement à l'épreuve théorique générale (ETG)</h2>
    ...
    <h2>A. 5. Epreuve théorique générale</h2>
    ...
    <h2>B. 3. Déroulement de la formation</h2>
    ...
    <h2>B. 4. Évaluation des compétences en fin de formation initiale</h2>
    ...
    <!-- Page 4 -->
    <h2>B. 5. Présentation à l'épreuve pratique du permis de conduire</h2>
    ...
    <h2>B. 6. Accompagnement à l'épreuve pratique</h2>
    ...
    <h2>A. Démarches administratives</h2>
    ...
    <h2>B. Inscription aux épreuves théorique et pratique du permis de conduire</h2>
    ...
    <table class="payement" style="border: 2px solid #393939;" >
        <tr>
            <td>
                {!! renderCheckbox('carte bancaire', true) !!}</td>
            <td>
                {!! renderCheckbox('chèque', false) !!}
            </td>
            <td>
                {!! renderCheckbox('virement', false) !!}
            </td>
            <td>
                {!! renderCheckbox('espèce', false) !!}
            </td>
        </tr>
    </table>
    <p>Le paiement pourra s'effectuer selon l'une des modalités suivantes : paiement comptant en un seul versement, échelonné en deux ou trois versements sans frais (paiement en ligne en 2 ou 3x sans frais)<br>
    Si la dernière option est retenue, les versements s'effectueront comme tel: premier versement le jour J et à J+30 pour le deuxième versement et à J+60 pour le troisième versement.</p>
    <br>
    <p>J'école de conduite délivre une note à l'élève avant le paiement de la prestation. Pour les prestations forfaitaires, la note indique la liste détaillée des prestations comprises dans le forfait. Conformément à l'article 1 de l'arrêté du 3 octobre 1983, toute prestation dont le prix est égal ou supérieur à 25 € TTC fera l'objet de la délivrance d'une note. Elle peut être remise sur simple demande de l'élève pour des prestations dont le prix est inférieur à 25 €.<br>
    En cas de défaillance de l'école de conduite, celle-ci n'a pas souscrit à un dispositif de garantie financière.</p>
    ...
    Opposition au démarchage téléphonique - En tant que consommateur, si l'élève ne souhaite pas faire l'objet de prospection commerciale par voie téléphonique, il est informé de son droit de s'inscrire gratuitement sur la liste d'opposition au démarchage téléphonique Bloctel sur le site internet : http://www.bloctel.gouv.fr ou par courrier Société Opposetel - Service Bloctel, 6, rue Nicolas-Siret, 10000 Troyes.</p>

    <p>Fait à Creil, le : {{ Carbon::parse($created_at)->format('d/m/Y')}} en deux exemplaires originaux</p>

    <table class="signature-table">
        <tr>
            <td  width="20%">Signature de l'élève</td>
            <td >Signature du représentant légal de l'élève mineur, le cas échéant</td>
            <td >Signature du responsable de l'école de conduite et cachet</td>
        </tr>
        <tr>
            <td style="height: 40px" class="center">
                <b> {{ $training['student']['user']['name'] }}</b>
                {{-- <img style="margin-top: -10px;z-index: 10000" src="{{$data['test_pro']['signature']}}" width="120" height="40"> --}}
            </td>
            <td class="center"></td>
            <td class="center relative pb-10">
                {{-- <img class="stamp" src="{{public_path('assets/AUTO-ECOLE-PASSPERMISFACILE.png')}}"> --}}
                <b> ETIENNE FLORIAN gérant de l’auto-école   PASSPERMISFACILE</b>
                <img  class="sign"  src="{{public_path('assets/signature.png')}}" >
            </td>
        </tr>
    </table>
</body>
