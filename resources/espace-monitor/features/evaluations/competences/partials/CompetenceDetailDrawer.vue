...
<template>
    <Drawer :show="!!item" :title="item?.label" @close="$emit('close')">
        <div class="flex flex-col p-4 gap-4 bg-dark-block mx-3">
            <div class="flex justify-between items-center gap-3">
                <p class="text-base font-medium flex gap-2">
                    <span class="w-7 h-7 p-1 bg-white/10 rounded-lg flex-center">
                        {{ item?.position }}
                    </span>
                    {{ item?.name }}
                </p>
        ...
        <ul class="mt-5 px-3 divide-y">
            <CompetenceItem v-for="v in item?.competencies" :key="v?.id" :item="v" :sid="student.id" />
        </ul>
    </Drawer>
</template>
