...
// Top-level admin check so template and actions can reuse it
const app = useApp();
...
const getActions = (item: UserType): BulkActionType[] | void => {
    // use top-level isAdmin computed defined above
    ...
    switch (Number(item.status)) {
        case GeneralStatusEnum.ACTIVE:
                const res: BulkActionType[] = [];

                // Only show Modifier to admins
                if (isAdmin.value) {
                    res.push({
                        label: 'Modifier',
                        icon: EditIcon,
...
// helper for template hrefs to avoid calling route() inside template expressions
const monitorRowHref = (item: UserType) => {
    try {
        return isAdminFlag && item.status !== GeneralStatusEnum.INACTIVE ? route(routes.users.monitors.edit, item.monitor.id) : '';
    } catch (e) {
        return '';
    }
};
