...
const onSubmit = async () => {
    try {
        ...
        if (isEdit.value) {
            await form.put(route(routes.api.reservations.update, state.selected?.id));
        } else {
            // @ts-ignore
            delete form.reservation_id;
            await form.post(route(routes.api.reservation.store));
        }
        emit('refetch');
        close();
    } catch (error) {
...
const onOffreChange = (item: WalletOffreType) => {
    ...
    if (item.offer && (item as any).offer.color) {
        form.color = (item as any).offer.color;
        return;
    }
    form.color = getReservationColor({
        student_type: (item as any).student_type || null,
        package_type: (item as any).offer?.package_type || (item as any).offer?.type_offre || null,
        evaluation_type: (item as any).offer?.evaluation_type || null,
        is_cpf: (item as any).offer?.is_cpf || false,
        offer: (item as any).offer || null,
    });
};
</script>

<template>
    <Drawer :show="state.show" width="md" :title="isEdit ? 'Modifier la reservation' : 'Nouvelle réservation'" @close="close">
        <form :key="form.key" class="flex-1 flex flex-col" @submit.prevent="onSubmit">
            <div class="px-3 border-y bg-white border-y-slate-300 flex flex-col gap-5 flex-1 py-5">
                ...
                />
                <ColorField v-model="form.color" label="Couleur de la Reservation" class="flex-none" />
                <Errors v-if="form.errors.message" :errors="form.errors.message" />
            </div>
            <div class="flex flex-row justify-between p-4 bg-gray-100 text-right rounded-b-2xl b sticky bottom-0">
