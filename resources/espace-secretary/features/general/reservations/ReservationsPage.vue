    ...
    <Page :actions="actions" :title="activeMonth().format('MMMM YYYY')" width="full">
        <template #center>
            <div class="flex-center gap-3">
                <ArrowLeftIcon
                    class="w-8 h-8 cursor-pointer hover:bg-gray-200 rounded-lg text-primary border border-dashed border-gray-400 p-2"
                    @click="onDateChange(-1)"
                />
                <MonthlyCountSchedule
                    v-if="filters.view === 'week'"
                    range
                    :week="filters"
                    :events="eventsQuery.data"
                    @change:week="eventsQuery.fetch('', $event)"
                    @update:week="
                        (v) => {
                            filters.start = v.start;
                            filters.end = v.end;
                        }
                    "
                >
                    <div class="filter-control min-w-60 flex-1 flex-center btn-m">
                        {{ `${filters.start} au ${filters.end}` }}
                    </div>
                </MonthlyCountSchedule>
                <DateField
                    v-else
                    class="flex-1 w-auto"
                    :key="activeMonth().format('yyyy-MM-DD')"
                    :model-value="activeMonth().format('yyyy-MM-DD')"
                    monthPicker
                    filter
                    @change="
                        (v) => {
