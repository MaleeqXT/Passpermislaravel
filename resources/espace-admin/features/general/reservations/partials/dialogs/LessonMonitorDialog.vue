...
const latestComment = computed(() => {
    ...
    return [...allComments.value].sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())[0];
});
...
<template>
    ...
    >
        <div class="p-3 flex gap-5 border-y border-slate-300 bg-white">
            <div class="flex-1">
                <!-- Student Info -->
                <div v-if="event?.training?.student" class="pb-3">
                    <StudentStatsCard
                    ...
                    <div class="p-3 bg-gray-50 rounded-lg">
                        ...
                        <template v-else>
                            ...
                            <div v-if="latestComment" class="mb-4">
                                ...
                                <p class="text-xs text-gray-400 mt-1">Ajouté le {{ dateFormat(latestComment.created_at, 'short') }}</p>
                            </div>
                            ...
                            <Button v-if="allComments.length > 0" variant="secondary" size="sm" class="mt-2" @click="showCommentDrawer">
                ...
                <Card title="Ajouter un commentaire" class="mt-4">
                    <form @submit.prevent="onSubmitComment">
                        ...
                        ></textarea>
                        <Button type="submit" variant="primary" size="sm" class="mt-2" :disabled="form.processing">
                            Enregistrer le commentaire
                        </Button>
                    </form>
                </Card>

                <Button v-if="event?.training" variant="secondary" full class="mt-1" @click="state.updateToDispo = true">
    ...
    <!-- ================= DRAWER ALL COMMENTS ================= -->
    <Drawer :show="stateLocal.showCommentDrawer" title="Tous les commentaires" max-width="md" @close="closeCommentDrawer">
        <div class="p-4">
            ...
            <div v-else class="space-y-4 max-h-96 overflow-y-auto">
                <div v-for="(comment, index) in allComments" :key="comment.id" class="border-b pb-4 last:border-b-0">
                    <div class="flex justify-between items-start mb-2">
                        <span class="text-sm font-medium text-gray-700"> Commentaire #{{ index + 1 }} </span>
