...
const getContractUrl = () => {
  ...
  const studentId =
    contract?.student?.id ?? // nested student object
    contract?.student_id ?? // flat field
    contract?.studentId ?? // camelCased field
    props.user?.student?.id ?? // from user prop
    props.user?.id ?? // fallback to user id
    contract?.id; // fallback to contract id
  ...
  // Try direct contract URL fields first
  ...
  // If we still don’t have an URL, build one from the known student id
  if (!url) {
    try {
      ...
      <Card block class="cursor-pointer card_wrap" @click="openEvaluation">
        ...
        >
          <p class="font-bold">
            {{
              evaluation
                ? "Voir le detail d'evaluations"
                : "Vous n'avez pas encore une évaluation pour cet condidat"
            }}.
          </p>
