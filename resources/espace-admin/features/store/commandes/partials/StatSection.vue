...
const stats = computed(() => {
    // when building stats for a cart, we expect the controller to pass
    // the list of related sales – log it so we can confirm the values
    if (props.sales && Array.isArray(props.sales) && props.sales.length > 0) {
        console.debug('StatSection props.sales', props.sales);
    ...
    // if we have a sale object and it's paid (either passed directly or
    // nested under a cart), show the recorded amounts from the sale model
    ...
    // default: derive from cart items
    return [
